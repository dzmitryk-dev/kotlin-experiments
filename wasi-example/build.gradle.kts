...
fun Project.createDenoExec(
    ...
    val outputDirectory = nodeMjsFile.map { it.asFile.parentFile.also { dname ->
        println("wasmFileName = $dname")
    } }
    val wasmFileName = nodeMjsFile.map {
        "${it.asFile.nameWithoutExtension}.wasm".also { fname ->
            println("wasmFileName = $fname")
        }
    }
